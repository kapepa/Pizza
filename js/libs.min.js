...
$(function(){
	...
	$(window).on("load", function(){
		$(".areaForLoader").delay(300).fadeOut(300)
	})

	$(".row-down").on("click", function(){
		let nextPos = $(this).closest("section").offset().top;
		$("html").animate({scrollTop: nextPos +"px"},1000)
	});

	$("#my-menu").mmenu({
		extensions: ["theme-dark","pagedim-black","border-none","fx-menu-slide","position-right","listview-50",],
		navbar:{title: '<a href="#" class="logo"><img src="img/Logo.svg" alt="Pizza"><div class="logo-desc"><span>Пицца</span><br>на дом</div></a>'}
	})
	...
	$(".select__pizza-item-content .h2").equalHeights();
	$(".select__pizza-item-content p").equalHeights();
	$(".select__pizza-item-content span").equalHeights();

//Размер Pizza
	var flag = true;
	$(".select__pizza-item-img-wrap").on("click","button", function(e){
		e.preventDefault();
		var $this = $(this),
				findId = $this.attr("data-id"),
				text = $this.addClass("active").siblings().removeClass("active").end();
		if(Goods[findId].price != $this.closest(".select__pizza-item-img-wrap").find("span").text()){
			if(flag){
				flag = false;
				$this.closest(".select__pizza-item-img-wrap").find("span").animate({opacity: 0},400);
		    	setTimeout(function(){
		    	$this.closest(".select__pizza-item-img-wrap").find("span").text(Goods[findId].price).animate({opacity: 1},400);
		    flag = true;
		    },500);	
			}
			$this.closest(".select__pizza-item").find(".button").attr("data-id",findId);
		}
	});

	$(".vegetable__carousel").owlCarousel({
		items: 3,
		loop: true,
		smartSpeed: 700,
		dots: true,
	...
	$(".vegetable__carousel-item-content .h2").equalHeights();
	$(".vegetable__carousel-item-content p").equalHeights();
	$(".vegetable__carousel-item-content div").equalHeights();

	$(".drink__carousel").owlCarousel({
		items: 3,
		loop: true,
		smartSpeed: 700,
		dots: true,
	...
	$(".order .h2").each(function(){
		var $this = $(this);
		$this.html($this.html().replace(/(\S+)\s*$/ig, "<span>$1</span>"));
	});
	...
	Goods = {
		1: new Products("Пицца Неаполитано 26см","10"),
		2: new Products("Пицца Неаполитано 32см","14"),
		3: new Products("Пицца Пепперони 26см","12"),
		4: new Products("Пицца Пепперони 32см","16"),
		5: new Products("Пицца Тропициана 26см","14"),
		6: new Products("Пицца Тропициана 32см","18"),
		7: new Products("Салат греческий 150г","5"),
		8: new Products("Салат морской 100г","7"),
		9: new Products("Салат с сыром фета и помидорами 150г","8,5"),
		10: new Products("Спрайт 1,5л","3"),
		11: new Products("Кока-кола 1,5л","3"),
		12: new Products("Фанта 1,5л","3"),
	}
	Cars = [];

	//init Cars
	function CheckBuy(){
		$(".order-items").empty().append('<span>Ваш заказ пуст :(</span>');
		var total = 0;
		for(var i = 0; i < Cars.length; i++){
			var elem = Cars[i],
					txt = '<b class="order-items-del" data-num="'+i+'"><i class="fa fa-times-circle"></i><p class="order-items-line">'+ dots(40, ". ", elem.name, elem.count + "шт./" + elem.price +" p."); +'</p></b>';
			function dots(leng, items, start, end){
				let dotAdd = leng - (start.length + start.length);
				for(let i = 0; i < dotAdd; i++){
					start += items;
				}
				start += end;
				return start;
			...
			$(".order-items span").empty();
			$(".order-items").append(txt);		
		}
		$(".valres").empty().append(total + " руб.");
		$(".order-items-del").on("click", function(){
			let itemDel = $(this).attr("data-num");
			Cars.splice(Cars[itemDel],1);
			CheckBuy();
		}); 	
	};CheckBuy();
	// В корзину
	...
	$("#main-form").submit(function() { //Change
		...
		$.ajax({
			...
			$(".success").addClass("active").hide().fadeIn(500);
			setTimeout(function() {
				$(".success").removeClass("active").fadeOut(500);
				th.trigger("reset");
			}, 3000);
		});
		return false;
	...
	$(".footer__gallery-content-item span").each(function(){
		var $this = $(this);
